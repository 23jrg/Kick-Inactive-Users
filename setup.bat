# Checks to see if the script was already ran and removes leftover files
if exist "C:\LogOffInactive" (@RD /S /Q "C:\LogOffInactive")

# Creates the resource folder
mkdir C:\LogOffInactive

# Changed the path to the working directory
pushd %~dp0

# Moves the powershell script and scheduled task to the resources folder
copy /y "LogInactiveOff.ps1" "C:\LogOffInactive"
copy /y "Log off inactive users.xml" "C:\LogOffInactive"

# Creates a shortcut to the uninstaller so the uninstaller can remove the resource folder without being inside it
...
#Creates the scheduled task to automatically trigger when someone logs in
